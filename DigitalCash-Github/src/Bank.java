...
public class Bank {
	...
	/*
	 * This method obtains an integer to be used during the unblinding process
	 * which will be used to determine the legitimacy of the various money
	 * orders.
	 */
	...
	/*
	 * This method determines the legitimacy of the various money orders that
	 * the bank is given.
	 */
	...
	/*
	 * Returns the public key to a calling function.
	 */
	...
	/*
	 * Returns the private key to a calling function.
	 */
	...
	/*
	 * Gets a list of bank signatures from a file and stores them in a signature
	...
	/*
	 * This function implements the RSA Digital Signature algorithm for the
	 * bank, it saves the digital signature to the end of the money order file.
	 * It first opens the file and then retrieves the data within, each line is
	 * run through the RSA algorithm and the private key and public keys are
	...
	public void makeBankDigitalSignature(MoneyOrder MO) throws IOException {
		...
		/*
		 * This will read each character individually and then the RSA function
		 * will be applied to the character (character is read as an int). The
		 * bank raises the message multiplied by the customer's random number by
		 * d. The result will be put into a string builder which will then be
		 * inserted into the file at the bottom of the file.
		 */
		...
		for (int i = 0; i < 3; i++) {
			...
			if (i == 1) {
				MO.setBankSig((int) Math.pow(MO.getUnqID(), d) % n, i);
			}
			if (i == 2) {
				if (i == 1) {
	...
	/*
	 * Verify the bank's signature given an unblinded money order.
	 */
	public static boolean verifyBankSignature(MoneyOrder MO) {
		...
		i++;
		if (MO.getUnqID() != (Math.pow(MO.getBankSig(i), e) % n)) {
			return false;
		}
		i++;
	...
	/*
	 * This method excepts a file name and a digital signature string and stores
	 * the digital signature on the file by appending it to the end of the file.
	 */

	public static void storeDigitalSignature(String file, String digitalSignature) throws IOException {
		...
		System.out.println("Digital Signature Written to: " + file);

	}

	/*
	 * A method that obtains all bank uniqueness strings from a file and stores
	 * them in the bankSignatureList string array.
	 */
